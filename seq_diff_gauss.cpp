...
int main(int argc, char** argv) {
    ...
    std::string outputPath = "output_dog.png";

    FileManager fileLoader(inputPath, "image");
    ...
    Image inputImg = convertToFloatImage(fileLoader);

    float sigma = 1.0f;
    float k = 1.6f;
    float tau = 1.0f; 
    
    Image resultImg = applyDoG(inputImg, sigma, k, tau);

    std::vector<unsigned char> byteData = convertToBytes(resultImg);
    

    FileManager fileSaver(byteData.data(), resultImg.width, resultImg.height, 1);
    
    if (fileSaver.saveImage(outputPath)) {
        std::cout << "sucess" << outputPath << std::endl;
    } 
    else {
        std::cerr << "failes" << std::endl;
    }
